...
const Section = ({ image, children,textM,textCenter, textLeft, textRight, flip, imageRight }) =>
  <Wrapper image={image} imageRight={imageRight}>
    <Text size={flip ? 1.2 : 2} align='left'>{textLeft}</Text>

    <Text size={flip ? .9 : 2.0} align='center'>{textCenter}</Text>

    <Text2 size={flip ? 1.2: 2} align='center'>{textM}</Text2>

    <Text size={flip ? 2 : 1.2} align={flip ? 'left' : 'right'}>{textRight}</Text>
    {children ? children : (
    <>
      {textLeft && <Text size={flip ? 1.2 : 1.5} align='center'>{textLeft}</Text>}
    </>
    )}
